...
(function (GT) {
  ...
  const TIPO_HOYOS = { '9': 9, '18': 18, 'par3': 9, 'completa': 18 };
  const TIPO_LABELS = { '9': '9 hoyos', '18': '18 hoyos', 'par3': 'Par 3', 'completa': 'Cancha completa' };
  ...
  function renderList(el, rounds) {
    ...
    el.innerHTML = `<div style="display:flex; flex-direction:column; gap: var(--space-3);">${rounds.map(roundCard).join('')}</div>`;
  ...
  function openRoundForm(rootView, roundId) {
    ...
    let holes = base.holes && base.holes.length ? base.holes.slice() : buildDefaultHoles(TIPO_HOYOS[base.tipo]);
    ...
    renderHolesEditor(holesEditor, holes);

    backdrop.querySelector('#f-tipo').addEventListener('change', (e) => {
      const n = TIPO_HOYOS[e.target.value];
      holes = buildDefaultHoles(n);
      renderHolesEditor(holesEditor, holes);
    });
    ...
    backdrop.querySelector('#btn-save-round').addEventListener('click', () => {
      readHolesFromEditor(holesEditor, holes);
      const computed = GT.engine.stats.computeRoundStats(holes);
      const payload = Object.assign({
        tipo: backdrop.querySelector('#f-tipo').value,
  ...
  function buildDefaultHoles(n) {
    const holes = [];
    for (let i = 1; i <= n; i++) {
      holes.push({ numero: i, par: 4, yardas: '', golpes: '', putts: '', fairway: false, green: false, penalty: false, arena: false, chip: false, upDown: false, sandSave: false, approach: '', observaciones: '' });
    }
    return holes;
  }

  function renderHolesEditor(el, holes) {
    el.innerHTML = `
  ...
  function readHolesFromEditor(el, holes) {
    holes.forEach((h) => {
      const row = el.querySelector(`[data-hole="${h.numero}"]`);
      if (!row) return;
      h.par = Number(row.querySelector('[data-h="par"]').value) || 4;
  ...
  GT.modules['rounds'] = { render: render };
})(window.GT = window.GT || {});
