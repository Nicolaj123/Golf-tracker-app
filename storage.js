...
(function (GT) {
  ...
  async function initFromSupabase(userId) {
    ...
    db.settings = settingsRow ? { theme: settingsRow.theme, units: settingsRow.units } : { theme: 'dark', units: 'metric' };
    if (!settingsRow) {
      await client.from(schema.COLLECTIONS.SETTINGS).upsert({ user_id: userId, theme: 'dark', units: 'metric' });
    }
