...
(function (GT) {
        ...
        <div class="card">
          ...
          <div class="field" style="margin-bottom: var(--space-4);">
            ...
            <select class="select" id="f-tipo">
              ...
              <option value="completa">Cancha completa</option>
            </select>
          </div>
  ...
  function emptyHole(numero) {
    return { numero: numero, par: 4, golpes: 0, putts: 0, fairway: false, green: false, penalty: false, arena: false, chip: false, upDown: false, sandSave: false };
  }
  ...
  function renderHoleScreen(container, draft) {
    ...
    container.querySelectorAll('[data-par]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        container.querySelectorAll('[data-par]').forEach((b) => b.classList.toggle('is-active', b === btn));
      });
    });
    ...
    container.querySelectorAll('[data-step]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        h[field] = Math.max(0, (h[field] || 0) + dir);
        container.querySelector('#val-' + field).textContent = h[field];
      });
    });
    ...
    container.querySelector('#btn-next-hole').addEventListener('click', () => {
      ...
      if (isLast) {
        ...
        draft.holeIndex++;
        draft.current = emptyHole(h.numero + 1);
        saveDraft(draft);
        renderHoleScreen(container, draft);
      }
    });
